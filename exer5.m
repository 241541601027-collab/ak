...
images = {image, contrast_stretched_img, equalized_img}; 
titles = {'Original', 'Contrast Stretched', 'Equalized'}; 
for i = 1:3 
subplot(3, 2, 2*i-1), imshow(images{i}), title([titles{i} ' Image']), axis off; 
subplot(3, 2, 2*i), imhist(images{i}), title([titles{i} ' Histogram']),xlabel('Pixel Intensity'), ylabel('Frequency'), grid on;
end
